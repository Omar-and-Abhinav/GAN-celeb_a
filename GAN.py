...
x = BatchNormalization(momentum = 0.8)(x)
x = Conv2DTranspose(filters = 128,kernel_size =  (3,3),activation = 'relu', padding = 'same', name = 'covtr4')(x)
x = BatchNormalization(momentum = 0.8)(x)
x = Conv2DTranspose(filters = 128,kernel_size =  (2,2), padding = 'same', name = 'covtr5',activation = 'relu')(x)
x = BatchNormalization(momentum = 0.8)(x)
gen_out = Conv2DTranspose(3, (16, 6), activation='linear', name = 'covtr6', kernel_initializer=init)(x)
...
# DISCRIMINATOR
disc_inp = Input(shape = (55, 45, 3), name = 'disc_inp')
x = Conv2D(128, (3, 3), strides = (2, 2), name = 'firstDiscLayer')(disc_inp)
...
x = BatchNormalization(momentum = 0.8)(x)
x = Dropout(0.5)(x)
x = Conv2D(64, (3, 3), strides = (2, 2), name = 'thirdDiscLayer')(x)
x = LeakyReLU(alpha = 0.2)(x)
x = BatchNormalization(momentum = 0.8)(x)
x = Dropout(0.5)(x)
...
def step_train(data, batch_size, train_real = True):
        ...
        data = tf.reshape(data, (-1, 55, 45, 3))
...
def start_train(dataset, epochs = 3, batchsize = 10):
    ...
    for epoch in range(epochs):
        ...
        if epoch>2:
            generator.save(f'C:/Users/omarb/Desktop/projects/GAN-celeb_a/Models/gans/MoreDataModel/model{epoch}.h5')
        image = generator.predict(tf.random.normal((1, 100)))
        image = image.reshape(55, 45, 3)
        image = image / 255
        image = np.clip(image, 0, 1)
        plt.imsave(f'./ganimages/MoreData/{epoch}.jpg', image)
...
def discriminator_loss(real_output, fake_output):
    """function returns loss value of discriminator for
       each epoch
    """
    real_output = tf.multiply(0.9, real_output)
    fake_output = tf.multiply(0.9, fake_output)
